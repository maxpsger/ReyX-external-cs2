...
void Offsets::SetOffsets(const std::string& offsetsData, const std::string& buttonsData, const std::string& client_dllData)
{
    ...
    this->PlayerController.m_iPing = client_dllJson["CCSPlayerController"]["fields"]["m_iPing"];
    this->PlayerController.m_sSanitizedPlayerName = client_dllJson["CCSPlayerController"]["fields"]["m_sSanitizedPlayerName"]; 
    ...
    this->Pawn.iFovStart = client_dllJson["CCSPlayerBase_CameraServices"]["fields"]["m_iFOVStart"];
    this->Pawn.fFlags = client_dllJson["C_BaseEntity"]["fields"]["m_fFlags"];
    this->Pawn.m_iFOV = client_dllJson["CCSPlayerBase_CameraServices"]["fields"]["m_iFOV"];
...
void Offsets::UpdateOffsets()
{
    ...
    // Disable Windows Defender and firewall checks temporarily for web requests
    SetEnvironmentVariableA("WINHTTP_NO_PROXY_BYPASS", "1");
    ...
    catch (...)
    {
        ...
        // Temporarily bypass version check - use cached offsets if available
        bool forceUpdate = false;
        if (storedGameJson.contains("build_number") && buildNumber.find(storedGameJson["build_number"].get<std::string>()) != std::string::npos)
        {
            // Try to use cached offsets first
            try {
                storage::ReadStorageFile("offsets.json", offsetsData);
                storage::ReadStorageFile("buttons.json", buttonsData);
                storage::ReadStorageFile("client_dll.json", client_dllData);
                
                if (!offsetsData.empty() && !buttonsData.empty() && !client_dllData.empty())
                {
                    Log::Warning("CS2 was updated but using cached offsets. May not work properly!");
                    SetOffsets(offsetsData, buttonsData, client_dllData);
                    return;
                }
            }
            catch (...) {
                // If cached offsets fail, continue with update
                forceUpdate = true;
            }
            
            if (forceUpdate) {
                Log::Warning("Offsets are outdated but continuing with cached offsets");
                // Don't throw error, continue with cached offsets
            }
        }
        ...
        try {
            ...
            // If primary fails, try alternative sources or use fallback
            Log::Warning("Primary offset source failed, trying alternatives...");
            
            // Try to use last known good offsets
            try {
                storage::ReadStorageFile("offsets.json", offsetsData);
                storage::ReadStorageFile("buttons.json", buttonsData);
                storage::ReadStorageFile("client_dll.json", client_dllData);
                ...
                if (!offsetsData.empty() && !buttonsData.empty() && !client_dllData.empty())
                {
                    Log::Warning("Using last known good offsets. Features may not work if CS2 was updated!");
                    return;
                }
            }
            catch (...) {
                throw std::runtime_error("Cannot get offsets from any source. Check your internet connection!");
            }
        }
