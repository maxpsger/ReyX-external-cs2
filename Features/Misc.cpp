...
namespace Misc
{
	...
	void ClientSideHitDetection(CEntity& LocalPlayer) noexcept
	{
		static bool wasShootingLastFrame = false;
		static std::chrono::steady_clock::time_point lastHitTime;
		static int framesSinceLastHit = 0;
		
		if (!MiscCFG::HitSound && !MiscCFG::HitMarker)
			return;
		
		if (!LocalPlayer.IsAlive() || LocalPlayer.Pawn.TeamID == 0)
			return;
		...
		{
			DWORD uHandle = 0;
			if (memoryManager.ReadMemory<DWORD>(LocalPlayer.Pawn.Address + Offset.Pawn.iIDEntIndex, uHandle))
			{
				if (uHandle != (DWORD)-1 && uHandle != 0)
				{
					DWORD64 listEntry = 0;
					DWORD64 entityAddress = 0;
					
					if (memoryManager.ReadMemory<DWORD64>(gGame.GetEntityListAddress() + 0x8 * ((uHandle & 0x7FFF) >> 9) + 0x10, listEntry) &&
					    memoryManager.ReadMemory<DWORD64>(listEntry + 0x78 * (uHandle & 0x1FF), entityAddress) &&
					    entityAddress != 0)
					{
						auto now = std::chrono::steady_clock::now();
						auto timeSinceLastHit = std::chrono::duration_cast<std::chrono::milliseconds>(now - lastHitTime).count();
	...
	{
		ClientSideHitDetection(LocalPlayer);
		return;
		
		/* OLD SERVER-SIDE CODE - NOT WORKING FOR EXTERNAL
		static int callCount = 0;
		static bool initialized = false;
		static int lastRoundNum = -1;
		callCount++;
		
		if (callCount <= 3 || callCount % 600 == 0) {
			Log::Info("[HITMANAGER] ==== Called! Count: " + std::to_string(callCount) + " ====");
			Log::Info("[HITMANAGER] HitSound enabled: " + std::string(MiscCFG::HitSound ? "YES" : "NO"));
			Log::Info("[HITMANAGER] HitMarker enabled: " + std::string(MiscCFG::HitMarker ? "YES" : "NO"));
			Log::Info("[HITMANAGER] TeamID: " + std::to_string(LocalPlayer.Pawn.TeamID));
			Log::Info("[HITMANAGER] MenuOpen: " + std::string(MenuConfig::ShowMenu ? "YES" : "NO"));
			Log::Info("[HITMANAGER] IsAlive: " + std::string(LocalPlayer.IsAlive() ? "YES" : "NO"));
		}
		
		if ((!MiscCFG::HitSound && !MiscCFG::HitMarker) || LocalPlayer.Pawn.TeamID == 0 || !LocalPlayer.IsAlive()) {
			if (callCount <= 3)
				Log::Info("[HITMANAGER] Returning early - conditions not met");
			return;
		}
		...
		// Debug addresses
		if (callCount <= 5 || callCount % 600 == 0) {
			Log::Info("[HITMANAGER] Pawn.Address: 0x" + std::to_string(LocalPlayer.Pawn.Address));
			Log::Info("[HITMANAGER] Offset.Pawn.BulletServices: 0x" + std::to_string(Offset.Pawn.BulletServices));
		}
		
		if (!memoryManager.ReadMemory(LocalPlayer.Pawn.Address + Offset.Pawn.BulletServices, pBulletServices))
		{
			Log::Error("[HITMANAGER] Failed to read BulletServices from 0x" + std::to_string(LocalPlayer.Pawn.Address + Offset.Pawn.BulletServices));
			return;
		}
		
		if (callCount <= 5 || callCount % 600 == 0) {
			Log::Info("[HITMANAGER] pBulletServices: 0x" + std::to_string(pBulletServices));
		}
		
		if (pBulletServices == 0) {
			if (callCount <= 5 || callCount % 600 == 0)
				Log::Error("[HITMANAGER] pBulletServices is NULL! Cannot read hit data.");
			return;
		}
			
		// Try different data sizes to find the correct one
		UINT8 byte_val = 0;
		UINT16 short_val = 0;
		UINT32 int_val = 0;
		
		if (callCount <= 10 || callCount % 600 == 0) {
			// Read different sizes for debugging
			memoryManager.ReadMemory<UINT8>(pBulletServices + Offset.Pawn.TotalHit, byte_val, sizeof(UINT8));
			memoryManager.ReadMemory<UINT16>(pBulletServices + Offset.Pawn.TotalHit, short_val, sizeof(UINT16));
			memoryManager.ReadMemory<UINT32>(pBulletServices + Offset.Pawn.TotalHit, int_val, sizeof(UINT32));
			
			Log::Info("[HITMANAGER DEBUG] Offset.Pawn.TotalHit = 0x" + std::to_string(Offset.Pawn.TotalHit));
			Log::Info("[HITMANAGER DEBUG] Reading from address: 0x" + std::to_string(pBulletServices + Offset.Pawn.TotalHit));
			Log::Info("[HITMANAGER DEBUG] UINT8 value: " + std::to_string(byte_val));
			Log::Info("[HITMANAGER DEBUG] UINT16 value: " + std::to_string(short_val));
			Log::Info("[HITMANAGER DEBUG] UINT32 value: " + std::to_string(int_val));
		}
		
		if (!memoryManager.ReadMemory<int>(pBulletServices + Offset.Pawn.TotalHit, totalHits, sizeof(int)))
		{
			Log::Error("[HITMANAGER] Failed to read TotalHit from 0x" + std::to_string(pBulletServices + Offset.Pawn.TotalHit));
			return;
		}

		// Check for invalid values (255 = 0xFF = uninitialized memory)
		if (totalHits == 255 || totalHits < 0 || totalHits > 1000) {
			if (callCount <= 5)
				Log::Error("[HITMANAGER] Invalid totalHits value: " + std::to_string(totalHits) + " - likely wrong offset or data type!");
			return;
		}
		
		// Initialize PreviousTotalHits on first run or when totalHits resets
		if (!initialized || (totalHits == 0 && PreviousTotalHits > 0)) {
			PreviousTotalHits = totalHits;
			initialized = true;
			Log::Info("[HITMANAGER] Reset PreviousTotalHits to: " + std::to_string(totalHits));
		}
		
		// Always log totalHits when both HitSound and HitMarker are enabled
		if ((MiscCFG::HitSound || MiscCFG::HitMarker) && (callCount <= 10 || callCount % 300 == 0)) {
			Log::Info("[HITMANAGER] TotalHits: " + std::to_string(totalHits) + ", PreviousHits: " + std::to_string(PreviousTotalHits));
		}
		
		// Check if hits increased (not just changed)
		if (totalHits > PreviousTotalHits && totalHits > 0) {
			int newHits = totalHits - PreviousTotalHits;
			Log::Info("[HITMANAGER] 🎯 HIT DETECTED! New hits: " + std::to_string(newHits) + ", Total: " + std::to_string(totalHits) + " (was " + std::to_string(PreviousTotalHits) + ")");
			
			if (MiscCFG::HitSound)
			{
				Log::Info("[HITMANAGER] Playing HitSound!");
				HitSound();
			}
			if (MiscCFG::HitMarker)
			{
				Log::Info("[HITMANAGER] Showing HitMarker!");
				hitMarker = HitMarker(255.f, std::chrono::steady_clock::now());
			}
		}
	...
	{
		if (!MiscCFG::BunnyHop ||  MenuConfig::ShowMenu || Local.Pawn.TeamID == 0)
			return;
	...
	void KnifeBot(const CEntity& Local, const std::vector<CEntity>& Entities, int autoKnifeKey) noexcept
	{
		...
		for (const auto& E : Entities)
		{
			if (!E.IsAlive())
				continue;
			// Use Pawn TeamID for team check
			if (E.Pawn.TeamID == Local.Pawn.TeamID && E.Pawn.TeamID != 0)
				continue;
	...
	void ZeusBot(const CEntity& Local) noexcept
	{
		...
		if (targetEntity.Pawn.Address == 0 || (MenuConfig::TeamCheck && targetEntity.Pawn.TeamID == Local.Pawn.TeamID && targetEntity.Pawn.TeamID != 0)) return;
